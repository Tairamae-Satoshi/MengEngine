...
uint32_t Color::R11G11B10F(bool RoundToEven) const
{
	...
	R.f = MathHelper::Clamp(m_value.f[0], 0.0f, kMaxVal) * kF32toF16;
	G.f = MathHelper::Clamp(m_value.f[1], 0.0f, kMaxVal) * kF32toF16;
	B.f = MathHelper::Clamp(m_value.f[2], 0.0f, kMaxVal) * kF32toF16;
...
uint32_t Color::R9G9B9E5() const
{
	...
	float r = MathHelper::Clamp(m_value.f[0], 0.0f, kMaxVal);
	float g = MathHelper::Clamp(m_value.f[1], 0.0f, kMaxVal);
	float b = MathHelper::Clamp(m_value.f[2], 0.0f, kMaxVal);

	// Compute the maximum channel, no less than 1.0*2^-15
	float MaxChannel = MathHelper::Max(MathHelper::Max(r, g), MathHelper::Max(b, kMinVal));

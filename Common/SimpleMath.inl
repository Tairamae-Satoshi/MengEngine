...
inline Vector3 Vector3::Normalized() const noexcept
{
	...
	XMVECTOR v1 = XMLoadFloat3(this);
	Vector3 result;
	XMStoreFloat3(&result, XMVector3Normalize(v1));
	return result;
}
...
inline Vector3 Vector3::Transform(const Vector3& v, const Matrix& m, bool coord) noexcept
{
	using namespace DirectX;
	XMVECTOR v1 = XMLoadFloat3(&v);
	XMMATRIX M = XMLoadFloat4x4(&m);
	XMVECTOR X;
	if (coord)
		X = XMVector3TransformCoord(v1, M);
	else
		X = XMVector3Transform(v1, M);
...
inline Vector3 Vector3::TransformVector(const Vector3& v, const Matrix& m) noexcept
{
	using namespace DirectX;
	Vector4 v1 = Vector4(v.x, v.y, v.z, 0.0f);
	Vector4 result = Vector4::Transform(v1, m);
...
inline Matrix Matrix::CreateAffineTransformation(const Vector3& scale, const Vector3& translation, const Quaternion& rotation) noexcept
{
	using namespace DirectX;
	Matrix R;
	XMStoreFloat4x4(&R, XMMatrixAffineTransformation(scale, Vector3::Zero, rotation, translation));
	return R;
}
...
inline Vector4 Quaternion::Ln() const noexcept
{
	using namespace DirectX;
	XMVECTOR q = XMLoadFloat4(this);
	return XMQuaternionLn(q);
}
...
inline Quaternion Quaternion::CreateFromVectors(const Vector3& v1, const Vector3& v2) noexcept
{
	using namespace DirectX;
	Vector3 v1_n = v1.Normalized();
	Vector3 v2_n = v2.Normalized();
	return CreateFromAxisAngle(v1_n.Cross(v2_n), acos(v1_n.Dot(v2_n)));
}

//inline float Quaternion::Distance(const Quaternion& q1, const Quaternion& q2) noexcept
//{
//	using namespace DirectX;
//	Quaternion q1_Inv;
//	q1.Inverse(q1_Inv);
//	Quaternion q1_Invxq2 = q1_Inv * q2;
//	Vector4 log = q1_Invxq2.Ln();
//	float distance = sinf(log.Length());
//}

inline Quaternion Quaternion::Exp(const Vector4& v) noexcept
{
	using namespace DirectX;
	XMVECTOR V = XMLoadFloat4(&v);
